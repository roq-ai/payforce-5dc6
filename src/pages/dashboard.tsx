...
import { getVerifications } from 'apiSdk/verifications';
import { VerificationInterface } from 'interfaces/verification';
import { getPaymentOptions } from 'apiSdk/payment-options';
import { PaymentOptionInterface } from 'interfaces/payment-option';

import { VerificationListPage as ShowCaseList } from 'pages/verifications';

function DashboardPage() {
  const { session } = useSession();

  const currencyStat = useSWR<PaginatedInterface<CurrencyInterface>>(
    () => `/currencies`,
    () => getCurrencies({ limit: 1, offset: 0 }),
  );

  const verificationStat = useSWR<PaginatedInterface<VerificationInterface>>(
    () => `/verifications`,
    () => getVerifications({ limit: 1, offset: 0 }),
  );

  const paymentOptionStat = useSWR<PaginatedInterface<PaymentOptionInterface>>(
    () => `/payment-options`,
    () => getPaymentOptions({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Currencies',
      result: currencyStat?.data?.totalCount,
      isLoading: currencyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Verifications',
      result: verificationStat?.data?.totalCount,
      isLoading: verificationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Payment Options',
      result: paymentOptionStat?.data?.totalCount,
      isLoading: paymentOptionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`verifications`} passHref legacyBehavior>
                  <Button
