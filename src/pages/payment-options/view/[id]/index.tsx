...
import { getPaymentOptionById } from 'apiSdk/payment-options';
import { PaymentOptionInterface } from 'interfaces/payment-option';
import { TransactionListPage } from 'pages/transactions';

function PaymentOptionViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<PaymentOptionInterface>(
    () => (id ? `/payment-options/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('payment_option', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/payment-options/edit/${id}`} passHref legacyBehavior>
                    <Button
