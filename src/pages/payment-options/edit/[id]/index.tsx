...
import { getPaymentOptionById, updatePaymentOptionById } from 'apiSdk/payment-options';
...
function PaymentOptionEditPage() {
  ...
  const handleSubmit = async (values: PaymentOptionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePaymentOptionById(id, values);
      mutate(updated);
      resetForm();
