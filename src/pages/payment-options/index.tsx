...
import { getPaymentOptions, deletePaymentOptionById } from 'apiSdk/payment-options';
import { PaymentOptionInterface } from 'interfaces/payment-option';

type ColumnType = ColumnDef<PaymentOptionInterface, unknown>;

interface PaymentOptionListPageProps {
...
export function PaymentOptionListPage(props: PaymentOptionListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PaymentOptionInterface>>(
    () => `/payment-options?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deletePaymentOptionById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: PaymentOptionInterface) => {
    if (hasAccess('payment_option', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/payment-options/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/payment-options/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('payment_option', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/payment-options/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('payment_option', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="payment_option" />
        </Flex>

        {hasAccess('payment_option', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/payment-options/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
